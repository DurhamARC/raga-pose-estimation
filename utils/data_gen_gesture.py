    ...
    for line in paths:
        ...
        for part in line:
            if part in row.index.levels[0]:
                if not np.isnan(row[part]).any() and len(np.where(row[part]>=1)[0])==3:
        ...
        cv2.polylines(
            img, [pts], False, (255, 255, 255), thickness=2,
        )
...
def generate_json(df, label, file):
    ...
    for ii in range(len(df)):
        ...
        skeleton = dict()
        skeleton['pose'] = [
                            row['RElbow']['x'], row['RElbow']['y'],
                            row['RWrist']['x'], row['RWrist']['y'],
                            row['LElbow']['x'], row['LElbow']['y'],
                            row['LWrist']['x'], row['LWrist']['y'],
                            ]
...
for anno in os.listdir(annotation_dir):
    ...
    for ii, start_frame in enumerate(range(0, int(num_frame-length), stride)): 
        ...
        # if rand < 9:
            json_file = os.path.join(output_train_dir, file+'_'+str(ii).zfill(5)+'.json')
            output_path = os.path.join(output_train_dir, file+'_'+str(ii).zfill(5)+'_'+label+'.mp4')
            ...
            json_file = os.path.join(output_val_dir, file+'_'+str(ii).zfill(5)+'.json')
            output_path = os.path.join(output_val_dir, file+'_'+str(ii).zfill(5)+'_'+label+'.mp4')
