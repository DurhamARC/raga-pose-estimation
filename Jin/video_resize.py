...
def video_resize(input_video_path, output_video_path):
    ...
    if os.path.exists(output_video_path):
        ...
        out = cv2.VideoWriter(output_video_path, fourcc, fps, (int(width/SCALE), int(height/SCALE)))
...
def video_clip(input_video_path, output_video_dir, length=10):
    ...
    if os.path.exists(output_video_dir):
        ...
        for ii, start_frame in enumerate(range(0, int(num_frame-length*fps), stride)):
            video_name = input_video_path.split('/')[-1]
            video_name = video_name.split('.')[0] + '_' + str(ii).zfill(3) + '.mp4'
...
if __name__ == "__main__":
    ...
    video_resize('../output/video_group/VK_Multani_CentralClose_3601_3719/video_overlay.mp4', '../output/video_group/VK_Multani_CentralClose_3601_3719/resize.mp4')
